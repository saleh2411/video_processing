...
def faster_lucas_kanade_step(I1: np.ndarray,
    ...
    corners = cv2.goodFeaturesToTrack(
        I2f.astype(np.float32), maxCorners=200, qualityLevel=0.01, minDistance=5)
    if corners is None:
        return du, dv
...
def faster_lucas_kanade_optical_flow(
    ...
    for level in range(num_levels, -1, -1):
        ...
        I2_warp = warp_image(I2_level, u, v) if (np.any(u) or np.any(v)) else I2_level

        level_iters = max_iter if level > 0 else 1   # finest = single pass
