...
def image_tiles_to_black_and_white_image(arr, h, w):
    ...
    Returns:
        (h, w) np.ndarray.
    Hint: Use only shape, reshape an(arr.reshape(h//nrows, nrows, -1, ncols)
               .swapaxes(1,2)
               .reshape(-1, nrows, ncols))d swapaxes to implement this method.
    Take inspiration from: https://stackoverflow.com/questions/16873441/form-a-big-2d-array-from-multiple-smaller-2d-arrays
    """
...
def create_grad_x_and_grad_y(input_image):
    ...
    if len(input_image.shape) == 2:
        ...
        input_image = cv2.cvtColor(input_image, cv2.COLOR_RGB2GRAY) #convert RGB image to grayscale image

    """INSERT YOUR CODE HERE.
    REPLACE THE VALUES FOR Ix AND Iy WITH THE GRADIENTS YOU COMPUTED.
    """
    gray = input_image.astype(np.float64)

    img_shifted_right = np.zeros_like(gray)
    img_shifted_right[:, 1:] = gray[:, :-1]
    Ix = gray - img_shifted_right

    shifted_down = np.zeros_like(gray)
    shifted_down[1:, :] = gray[:-1, :]
    Iy = gray - shifted_down
...
def our_harris_corner_detector(input_image, K, threshold):
    ...
    rh, rw = response_image.shape
    padded_response = np.zeros((H, W), dtype=response_image.dtype)
    padded_response[H - rh:, W - rw:] = response_image

    Hc = (H // TILE) * TILE
    Wc = (W // TILE) * TILE
    cropped = padded_response[:Hc, :Wc]

    tiles = black_and_white_image_to_tiles(cropped, TILE, TILE)

    nms_tiles = np.zeros_like(tiles)
    for i in range(tiles.shape[0]):
        tile = tiles[i]
        r, c = np.unravel_index(np.argmax(tile), tile.shape)
        nms_tiles[i, r, c] = tile[r, c]

    nms_image = image_tiles_to_black_and_white_image(nms_tiles, Hc, Wc)

    output_image = np.zeros((H, W), dtype=np.uint8)
    output_image[:Hc, :Wc] = (nms_image > threshold).astype(np.uint8)
    return output_image
